...
- FrontPad Bridge **1.3.6**: запятые с пробелом внутри добавки («Капуста, томаты») больше не режут заказ на несколько этикеток; фрагменты склеиваются обратно в одну позицию.
- FrontPad Bridge **1.3.5**: имена с запятыми внутри добавок («Овощи свежие. Капуста, томаты…») больше не порождают 7 этикеток «Позиция N»; пустые слоты отбрасываются.
- Редактор шаблонов: режим **«Превью печати»** рисует тот же Skia-PNG, что уходит на принтер (раньше Avalonia-превью сильно отличалось).
- Редактор шаблонов: размер шрифта в векторном режиме ближе к печати (`pt × dpi/72`).
- Линия в шаблоне: высота по умолчанию 0 (горизонтальная); раньше «наклон» при Rotation=0 из‑за Height=8.
- Линия в редакторе: поворот вокруг середины отрезка (без «кривого» смещения из‑за увеличенной hit-зоны).
- Диалог подтверждения (удаление и т.п.): стабильная ширина и выравнивание кнопок.
- Настройки → Принтеры: форма редактора прокручивается, кнопки **Сохранить/Отмена** всегда видны на низком экране.
- Таблицы (каталог, заказы, шаблоны, принтеры, очередь, история): колонки лучше сжимаются, есть горизонтальный скролл.
- Заказы → позиции: под названием показываются добавки из комментария.

### Changed

- Редактор шаблонов: свойства форм — пунктир линии, толщина обводки, скругление прямоугольника; элемент **Иконка** с выбором/импортом и цветом **чёрный/белый**.
- Редактор шаблонов: белый текст/фигуры (`Invert`), заливка, тип «Текст/Переменная», кнопка **Добавки** (AddonsKitchen) — можно собрать кухонный чек без правки JSON.
- Каталог: только пользовательские PNG-иконки (встроенный набор убран из списков); подкатегории маркировки не сидятся — создаются вручную; старые сиды архивируются при старте.

### Added

- Настройки → Шаблоны: **Импорт JSON** (пара к экспорту).
- Маркировка: иконки у позиций (как у добавок) — выбор/импорт PNG в **Каталог → Маркировка**, переменная `ProductIconKey`, иконка на шаблоне.
- Системный шаблон **«Маркировка 58×40»** по единому стилю: иконка + название, ИЗГОТОВЛЕНО / ИСПОЛЬЗОВАТЬ ДО / ХРАНИТЬ ПРИ, пунктирные разделители.
## [0.9.1] - 2026-08-04
